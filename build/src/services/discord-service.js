...
import { getBalance, getNftsForOwner, stopTracking, trackAddress, } from './alchemy-service.js';
        ...
        if (command === '!track') {
            ...
            try {
                ...
                if (nfts.ownedNfts.length === 0) {
                    message.channel.send(`🎨 No NFTs found for address [${shortenAddress(address)}](${getEtherscanUrl(Network.ETH_SEPOLIA, address)})`);
                }
                else {
            ...
            stopTracking();
            message.channel.send(':pause_button: Wallet tracking has stopped.');
        }
